        ...
        while queue:
            ...
            for v in self.get_adjacent_nodes(G, u):
                ...
                # Check for equal cost path AND equal number of hops.
                    ...
                    # calculation and compared with "u".
                    v2 = self.get_path(v)[-2]

                    # Check all 8 Bytes of BridgeID, to find the first
                    # tiebreaker for ECT.
                    for i in range(8):
                        if d_node.get(v2)["BridgeID"][i][0] != \
                           d_node.get(u)["BridgeID"][i][0]:
                            xor_v = self.byte_xor(
                                self, d_node.get(v2)["BridgeID"][i][0],
                                d_ect.get(bvl))
                            xor_u = self.byte_xor(
                                self, d_node.get(u)["BridgeID"][i][0],
                                d_ect.get(bvl))
                            self.ectcnt += 1
                            if xor_v != xor_u:
                                break
...
def menu3():
    ...
    try:
        if input3 == "y" or input3 == "Y":
            ...
            print("\nNumber of different pathes available " +
                  "(if 256 BVLAN`s/ECT`s were used): ")
...
def menu30():
    for k in d_node:
        print("Node {:>{width}} performed {:>4d} ECT Calculations \
(XOR calculations) per BVLAN - hence in total: {}".format(
                 k, d_dijkstra[k][4000].ectcnt,
                 d_dijkstra[k][4000].ectcnt*len(set_bvls_in_use),
                 width=max_nodename))


@decorator_cls_and_return_mainmenu
def menu31():
    print("-> Time for all-pair-shortest-path calculation: {}".format(
