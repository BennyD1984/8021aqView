...
# GraphView 1.2 - bug fix: Tandem ISID calculation aborts, if one node contains 
# --------------------------the name of another i.e. "Node-1" and "Node-1-B".														   
...
import re # Bugfix multicast tandem mode, nodes with same name.
...
def menu43():
    ...
    print("Resizing factor more than 1 means, bigger lines")
    print("------------------------------------------------")
...
low_bvl_to_test! Specify value between 4000 and 4256"
    ...
    # d_node["DC1_L1"]["ISID"].append([14001, 4001, 1, 1])
    # d_node["DC2_L1"]["ISID"].append([14001, 4001, 1, 1])
    # d_node["DC2_SS8"]["ISID"].append([8000, 4005, 1, 0])
    # d_node["DC1_S10"]["ISID"].append([14001, 4001, 1, 1])
        ...
        for isid in d_tandem_mcast_src[mc_src_a]:  # For every isid.       
            for bvl, mc_rcv_b in product(
             d_tandem_isid[isid], d_tandem_mcast_rcv):
                if isid in d_tandem_mcast_rcv[mc_rcv_b]:
                    # Convert path to string, for RegEx search
                    path_as_string_0_join = ",".join(
                        d_dijkstra[mc_src_a][bvl].get_path(mc_rcv_b))
                    # PreCheck, if Node anywhere Path:
                    if re.search(regex1, path_as_string_0_join):
                        path_as_string_1_join = ",".join(
                            d_dijkstra[mc_src_a][bvl].get_path(
                                mc_rcv_b)[1:-1])
                        len_path = len(d_dijkstra[mc_src_a][bvl].get_path(
                            mc_rcv_b))
                        # First check if node is in BETWEEN path.
                        # Second check if node is START or path.
                        # Third check if node is END of path.
                        if re.search(regex1, path_as_string_1_join):
                            pos_node = d_dijkstra[mc_src_a][bvl].get_path(
                                mc_rcv_b).index(node)
                            iil = d_dijkstra[mc_src_a][bvl].get_path(
                                mc_rcv_b)[pos_node-1]
                            oil = d_dijkstra[mc_src_a][bvl].get_path(
                                mc_rcv_b)[pos_node+1]
                            d_mcast_states[node].append(
                                [mc_src_a, isid, bvl, iil, oil])
                        elif node == d_dijkstra[mc_src_a][bvl].get_path(
                                mc_rcv_b)[0] and len_path > 1:
                            pos_node = d_dijkstra[mc_src_a][bvl].get_path(
                                mc_rcv_b).index(node)
